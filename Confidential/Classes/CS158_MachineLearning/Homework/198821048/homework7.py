...
def scale_column(col):
    return Scaler(col).scale(col)

def unscale_column(col):
    return Scaler(col).unscale(col)

# %%
X_train_scaled = np.transpose(X_train)
X_test_scaled = np.transpose(X_test)

# %%
for i in range(len(X_train_scaled) - 1):
    X_train_scaled[i] = scale_column(X_train_scaled[i])
    
for i in range(len(X_train_scaled) - 1):
    X_test_scaled[i] = scale_column(X_test_scaled[i])
    
X_train_scaled = np.transpose(X_train_scaled)
X_test_scaled = np.transpose(X_test_scaled)

# %%
y_train_scaled=scale_column(y_train)
